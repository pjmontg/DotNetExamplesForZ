// -------------------------------------------------------------
// <copyright company="Pipeline Group, Inc." file="AbstractPlugin.cs">
// COPYRIGHT NOTICE
...
// <remarks>
//   $HeadURL: https://pipesvn.pipelinenetwork.com/svn/STP/Product/trunk/WebApp/PluginAPI/Base/AbstractPlugin.cs $
//   $LastChangedRevision: 10641 $ 
//   $LastChangedDate: 2014-02-19 15:14:06 -0800 (Wed, 19 Feb 2014) $ 
//   $LastChangedBy: pmontgomery $ 
// </remarks>  
// <summary>  
//   Abstract example for the inherited plugins
// </summary> 
// ------------------------------------------------------------- 
...
namespace PluginAPI.Base
{
    ...
    {
        private Queue<IPluginInstallerCommand> _installCommands;
        
        public Queue<IPluginInstallerCommand> InstallerCommands
        {
            get
            {
                if (null == _installCommands)
                {
                    _installCommands = new Queue<IPluginInstallerCommand>();
                    this.RegisterInstallerCommands();
                }
                return _installCommands;
            }
        }

        public IPluginStateCommand StateCommand { get; protected set; }

        protected abstract void RegisterInstallerCommands();
        protected abstract void RegisterStateCommand(IPluginService pluginService);


        public virtual void Initialize(IPluginService pluginService)
        {
               RegisterStateCommand(pluginService);
        } 
    }
}
